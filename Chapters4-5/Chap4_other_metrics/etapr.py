...
import math
import copy
import Chap4_other_metrics.etapr_utils as etapr_utils
import numpy as np
...
class eTaPR(TaPR):
    ...
    def eTaR(self) -> float:
        ...
        return np.mean((detection_scores + detection_scores * portion_scores) / 2)

    def _etap_d(self, theta: float):
        if self.get_n_anomalies() == 0.0 or self.get_n_predictions() == 0.0:
            return 0.0, []
    ...
    def eTaP_d(self):
        ...
        tapd = 0.0
        if float(self._predictions_total_weight):
